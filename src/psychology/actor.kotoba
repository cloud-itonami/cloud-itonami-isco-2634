(ns psychology.actor
  "PracticeActor — the ISCO-08 2634 independent psychology actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one practice operation request
  ...
  The unconditional invariant: the PracticeAdvisor can never directly
  commit a record or dispatch a robot action the PsychologyGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [psychology.advisor :as advisor]
            [psychology.governor :as governor]
            [psychology.store :as store]))

(defn build-graph
  "Build a compiled PracticeActor graph. `store` implements
  `psychology.store/Store`. `advisor` implements
  `psychology.advisor/Advisor` (defaults to `mock-advisor`).
